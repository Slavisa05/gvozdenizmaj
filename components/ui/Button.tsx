...
const Button = ({ type = 'button', text, icon: Icon, variant = 'primary', onClick, disabled, isLoading, loadingText }: ButtonProps) => {
    return(
        <button type={type} onClick={onClick} disabled={disabled || isLoading}
                className={`relative overflow-hidden font-bold uppercase py-3 px-5 rounded-xl cursor-pointer transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0 inline-flex items-center justify-center disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0 ${variantStyles[variant]}`}>
